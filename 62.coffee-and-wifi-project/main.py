...
class CafeForm(FlaskForm):
    ...
    closing_time = StringField('Closing Time e.g. 4PM', validators=[DataRequired()])
    ...
    power_outlet_rating = SelectField('Power Socket Availability', validators=[DataRequired()], choices=['❌', '💪', '💪💪', '💪💪💪', '💪💪💪💪', '💪💪💪💪💪'])
...
def add_cafe():
    ...
    if form.validate_on_submit():
        with open('cafe-data.csv', mode='a', newline='') as csv_file:
            ...
            writer.writerow(list(form.data.values())[0:7])
                        '''ALTERNATIVE WAY 
                           csv_file.write(f"\n{form.cafe.data},"
